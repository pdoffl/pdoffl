# 👋 Hello there, I'm Pratik Das

Welcome to my GitHub profile.

**Red Team Consultant | Penetration Tester | Security Researcher**

> *"I help organizations understand their security posture by attacking it."*

I'm an offensive security professional with 3 years of hands-on experience conducting red team operations, penetration testing, and adversarial emulation across enterprise, government, and industrial control environments. Graduated with MS in Cybersecurity from Northeastern University.

Previously: Security Consultant at **Ernst & Young**, Cybersecurity Consultant for **Government of India** (commended by PM's Office 2021), and Cybersecurity Consultant Co-op at **Factory Mutual**.

---

## 🎯 Core Expertise

```text
|── Offensive Security
│   ├── Red Team Operations & Adversarial Emulation (MITRE ATT&CK)
│   ├── Active Directory Exploitation (Kerberoasting, ADCS, ACL abuse)
│   ├── Web Application Penetration Testing (SQLi, XSS, RCE)
│   ├── C2 Operations (Cobalt Strike, Sliver, custom frameworks)
│   └── Post-Exploitation (Lateral Movement, Privilege Escalation, Persistence)
│
├── Security Research & Development
│   ├── Custom C2 Framework Development (ARM64/Linux)
│   ├── Exploit Development (Buffer overflows)
│   ├── Malware Analysis & Reverse Engineering
│   └── ICS/OT Security Assessment
│
└── Education & Content Creation
    ├── CTF Challenge Design & Infrastructure
    ├── Technical Blog Writing
    └── Security Lab Design Automation (Docker, Bash, Python, Virtual Machines)
```

---

## 🛠️ Technical Arsenal

**Languages & Frameworks**
```
C/C++ • Python • ARM64/Aarch64 Assembly • x86/x64 Assembly • Go • PowerShell • Bash • C#
```

**Offensive Security Tools**
```
Cobalt Strike • Metasploit • Sliver C2 • BloodHound • Mimikatz • Impacket • CrackMapExec • NetExec • Burp Suite • SQLMap • Nmap • Responder • Empire • Covenant • Havoc C2
```

**Specializations**
```
Active Directory Security • Web Application Security • Binary Exploitation
ICS/OT Security • Wireless Security (WPA2, EAP) • Cloud Security (AWS/GCP)
Container Security (Docker, Kubernetes) • SIEM Operations (Splunk)
```
---

## 📝 Publications & Research

### 📚 Technical Blogs
- Check out some write-ups [here](https://github.com/pdoffl/Write-Ups).
- My medium blog: https://a-learner.medium.com/


### 📄 Academic Publication
- **[Significance of Cloud Security Policies and Practices in Corporations and Organizations](https://www.researchtrend.net/ijet/pdf/48%20Significance%20of%20Cloud%20Security%20Policies%20and%20Practices%20in%20Corporations%20and%20Organizations-2931-CSE-Nishant%20Kumar.pdf)** - IJET 11(5): 320-329 (2020)

### 🎓 Capstone Research
- **Assessing PiKVM as a Threat in Modern Organizations** - Analyzed weaponization of Raspberry Pi-based KVMs by APT groups, developed 5+ detection/remediation guidelines.

---

## 🏆 Certifications & Achievements

**Offensive Security Certifications**
- 🔴 **OSCP** - Offensive Security Certified Professional
- 🔴 **CRTO** - Certified Red Team Operator (Zero-Point Security)
- 🔴 **CRTP** - Certified Red Team Professional (Altered Security)
- 🔵 **CEH** - Certified Ethical Hacker
- 🔵 **ECSA** - EC-Council Certified Security Analyst
- 🟢 **PT1** - TryHackMe Junior Penetration Tester

**Recognition**
- 🏅 **PM's Office Commendation** (July 2021) - For offensive security and adversary emulation excellence.
- 🥉 **3rd Place** - Malware Analysis CTF, Northeastern University (Spring 2025)

---

## 🤝 Let's Connect

I'm always interested in discussing offensive security, red team operations, exploit development, and security research.

- 💼 **LinkedIn**: [linkedin.com/in/pratikdas-cy](https://linkedin.com/in/pratikdas-cy)
- 📝 **Medium**: [a-learner.medium.com](https://a-learner.medium.com)
- 📧 **Email**: pratikdasofficial@outlook.com

